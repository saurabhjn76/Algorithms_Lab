...
// defining positive auxillarly constants for quardratic probing
 #define C1 2
 #define C2 5
 ...
 int quardratic_probing(int ke,int i){
 	//hash function=ke%size;
 	//new hash function
 	int new_index=((ke%size)+C1*i+ C2*i*i)%size;  // quardratic probing
 	return new_index;
 ...
 void insert( int table[],int ke,int i)
 {
 	...
 	new_index=quardratic_probing(ke,prob_i);
 ...
 {
 	int prob_i=0;
 	int new_index=quardratic_probing(ke,prob_i);
